...
export default function Memories({ onNext }) {
  ...
  const randomizedPhotos = useMemo(() => {
    ...
    // THE FIX: Grab both the original folder path AND the new live URL generated by Vite
    const allEntries = Object.entries(allImages);
    
    const albumPhotos = [];

    allEntries.forEach(([originalPath, liveUrl]) => {
      // Decode the original path to safely check the folder name
      const decodedPath = decodeURIComponent(originalPath);
      
      // If the original path has the right folder, AND the live URL isn't the cover image
      if (decodedPath.includes(`/${albumData.folderName}/`) && liveUrl !== albumData.coverImage) {
         albumPhotos.push(liveUrl); // Push the live URL to be displayed!
      }
    });
