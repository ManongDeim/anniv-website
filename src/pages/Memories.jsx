...
export default function Memories({ onNext }) {
  ...
  // Because it is at the top level, React is happy. It will only recalculate when selectedAlbum changes.
  const randomizedPhotos = useMemo(() => {
    // Safety check: if she is on the main grid (no album selected), return nothing
    if (!selectedAlbum) return [];

    const albumData = albums.find(a => a.id === selectedAlbum);
    if (!albumData) return [];

    const allPaths = Object.values(allImages);
    
    // Filter and fix the %20 space issue
    const albumPhotos = allPaths.filter(path => {
      const decodedPath = decodeURIComponent(path);
      return decodedPath.includes(`/${albumData.folderName}/`) && decodedPath !== albumData.coverImage;
    });

    // Map the random positions
    return albumPhotos.map(photoSrc => ({
      src: photoSrc,
